...
if ($recoveryModel -eq 'Full') {
	...
	foreach ($s in $TargetServer) {
		...
		Write-Verbose "Initiating database restore`: $backupFileLocation to $s.$targetDatabaseName"
	...
	Write-Verbose "Initiating database restore`: $backupFileLocation to $currentPrimary.$targetDatabaseName"
	$restore = $backup.Path | Restore-DbaDatabase -SqlInstance $conn -DatabaseName $targetDatabaseName -WithReplace -ReplaceDbNameInFile -EnableException
	...
	# Adding Start-Sleep here because apparently our backup storage is slow and don't see the file immediately after it's created
	Start-Sleep 5
	Write-Verbose "Removing backup file $($backup.Path)"
	Remove-Item -LiteralPath $backup.Path -ErrorAction Stop
